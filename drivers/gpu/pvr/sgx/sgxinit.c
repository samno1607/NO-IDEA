...
unsigned int userRequestSGXInfo = 0;//NATTING_TEST//for NEW DDK and apps.
#endif
...
#if defined(SUPPORT_HW_RECOVERY)
  if (userRequestSGXInfo == 1)//NATTING_TEST//for NEW DDK and apps.
  {
    printk("user request SGXDumpDebuginfo!!!Skip sgx recovery!!\n");
  }
  else
  {
    sgx_manual_recovery = 1;
    printk(" ===================================================\n");
    printk(" SGXDumpDebugInfo()::PVR manual recovery set!!! for cosmo lock-up issue \n");
    printk(" ===================================================\n");
  }
#endif
...
#if defined(SUPPORT_HW_RECOVERY)
  printk(" ===================================================\n");
  printk(" PVR manual recovery START!!!!!!! \n");
  printk(" ===================================================\n");
...
#error non-mp perpipe regs not yet supported
    ...
    #if defined(SUPPORT_HW_RECOVERY)//NATTING_TEST//for NEW DDK and apps...need a mutex??
//			userRequestSGXInfo = 1;
    #endif
    
			SGXDumpDebugInfo(psDeviceNode->pvDevice, IMG_FALSE);

    #if defined(SUPPORT_HW_RECOVERY)//NATTING_TEST//for NEW DDK and apps.
//      userRequestSGXInfo = 0;
    #endif
